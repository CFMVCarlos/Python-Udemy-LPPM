...
def btn_click(char):
    if char == "=":
        ...
        try:
            ...
            tkinter.messagebox.showerror("Error", "Your calculation is invalid.")
        except ZeroDivisionError:
            tkinter.messagebox.showerror("Error", "You cannot divide by zero.")
        else:
...
for key_row in keys:
    ...
    for key in key_row:
        btn = tk.Button(
            keypad, text=key[0], width=2, command=lambda char=key[0]: btn_click(char)
        )
...
main_window.update()
main_window.minsize(
    keypad.winfo_width() + main_window_padding * 2,

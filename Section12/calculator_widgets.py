...
class CopyLabel(tk.Label):
    ...
    def on_copy(self, event):
        ...
        tkinter.messagebox.showinfo("Clipboard", "text copied")
...
class CalculatorGrid(tk.Frame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self.result = CopyLabel(
            self, borderwidth=2, relief="sunken", anchor="w", bg="white"
        )
        ...
        for key_row in CalculatorGrid.keys:
            ...
            for key in key_row:
                btn = CalculatorButton(
                    keypad, text=key[0], width=2, callback=self.on_click
                )
            ...
            if self.result["text"]:
                try:
                    ...
                    tkinter.messagebox.showerror(
                        "Error", "Your calculation isn't valid."
                    )
                except ZeroDivisionError:
                    tkinter.messagebox.showerror("Error", "You can't divide by zero.")
                else:
